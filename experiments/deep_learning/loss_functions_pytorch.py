...
def breslow_likelihood_torch( log_partial_hazard: torch.Tensor,y: torch.Tensor) -> torch.Tensor:
...
class BreslowLoss(_Loss):
    ...
    def forward(self, prediction, input):
        loss = breslow_likelihood_torch( prediction,input)
        return loss.to(torch.float32)
    

# Efron  loss

def efron_likelihood_torch(log_partial_hazard: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
...
class EfronLoss(_Loss):
    ...
    def forward(self, prediction, input):
        loss = efron_likelihood_torch(prediction,input)
...
def ah_likelihood_torch(
    ...
    event_mask = event.bool()
    rv = torch.distributions.normal.Normal(0, 1, validate_args=None) 
    #from None to False, but doesn't help integral quad
    sample_repeated_linear_predictor = (
